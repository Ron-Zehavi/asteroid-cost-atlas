...
function earthPosAt(dayOffset: number) {
  ...
  const ecl = keplerToCartesian({ ...EARTH, ma });
...
export function TransferArc({ asteroid, dayOffset }: Props) {
  const scene = useMemo(() => {
    const a = asteroid;
    ...
    if (mission.phase === 'in_transit') {
      ...
      return {
        ...
        labelPos: new THREE.Vector3(shipPos.x, shipPos.y + 0.12, shipPos.z),
  ...
  if (scene.phase === 'waiting') {
    return (
      <Html position={scene.labelPos} center style={{ pointerEvents: 'none' }}>
        <div style={labelStyle('#555566')}>
  ...
  if (scene.phase === 'arrived') {
    return (
      <group>
        ...
        <Html position={[scene.pos.x, scene.pos.y + 0.08, scene.pos.z]} center style={{ pointerEvents: 'none' }}>
          <div style={labelStyle('#44ff44')}>
